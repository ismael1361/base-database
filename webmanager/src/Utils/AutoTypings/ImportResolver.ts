...
import { ImportResourcePath, ImportResourcePathPackage, ImportResourcePathRelativeInPackage, importResourcePathToString } from "./ImportResourcePath";
...
export class ImportResolver {
	...
	private async resolveImportFromPackageRoot(importResource: ImportResourcePathPackage): Promise<ImportResourcePathRelativeInPackage | undefined> {
		...
		if (pkgJson) {
			...
			if (pkg.typings || pkg.types) {
				...
				const typingPackageName = `@types/${importResource.packageName.startsWith("@") ? importResource.packageName.slice(1).replace(/\//, "__") : importResource.packageName}`;
	...
	private async loadSourceFileContents(importResource: ImportResourcePathRelativeInPackage): Promise<{ source: string; at: string } | null> {
		...
		if (appends.map((append) => importResource.importPath.endsWith(append)).reduce((a, b) => a || b, false)) {
			...
			for (const append of appends) {
				...
				const fullPath = (append === ".d.ts" && resourcePath.endsWith(".js") ? resourcePath.slice(0, -3) : resourcePath) + append;
